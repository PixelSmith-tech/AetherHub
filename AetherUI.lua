...
local Aether = { Folder = "Aether", Options = {}, ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(117, 164, 206)), ColorSequenceKeypoint.new(0.50, Color3.fromRGB(123, 201, 201)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(224, 138, 175))} }
...
local AetherUI = isStudio and script.Parent:WaitForChild("Aether UI") or game:GetObjects("rbxassetid://86467455075715")[1]
...
local function Hide(Window, bind, notif)
	...
	bind = bind[2]
	if notif then
		Aether:Notification({Title = "Interface Hidden", Content = "The interface has been hidden, you may reopen the interface by Pressing the UI Bind In Settings ("..tostring(bind)..")", Icon = "visibility_off"})
	end
...
if gethui then
	AetherUI.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(AetherUI)
	AetherUI.Parent = CoreGui
elseif not isStudio and CoreGui:FindFirstChild("RobloxGui") then
	AetherUI.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == AetherUI.Name and Interface ~= AetherUI then
...
AetherUI.SmartWindow.Visible = false
AetherUI.Notifications.Template.Visible = false
AetherUI.DisplayOrder = 1000000000

local Main : Frame = AetherUI.SmartWindow
local Dragger = Main.Drag
local dragBar = AetherUI.Drag
...
local Notifications = AetherUI.Notifications
...
-- 	for FlagName, Flag in pairs(Aether.Flags) do
...
-- 		else
-- 			notified = true
-- 			Aether:Notification({Title = "Config Error", Content = "Aether was unable to load or find '"..FlagName.. "'' in the current script. Check ".. website .." for help.", Icon = "flag"})
-- 		end
-- 	end
-- 	if autoload and notified == false then
-- 		Aether:Notification({
-- 			Title = "Config Autoloaded",
-- 			Content = "The Configuration Has Been Automatically Loaded. Thank You For Using Aether Library",
-- 			Icon = "file-code-2",
-- 			ImageSource = "Lucide"
-- 		})
...
-- 		Aether:Notification({
-- 			Title = "Config Loaded",
-- 			Content = "The Configuration Has Been Loaded. Thank You For Using Aether Library",
-- 			Icon = "file-code-2",
-- 			ImageSource = "Lucide"
-- 		})
-- 	end
...
function Aether:Notification(data) -- action e.g open messages
	task.spawn(function()
		data = Kwargify({
			Title = "Missing Title",
...
function Aether:CreateWindow(WindowSettings)

	WindowSettings = Kwargify({
		Name = "Aether UI Example Window",
		Subtitle = "",
		LogoID = "6031097225",
		LoadingEnabled = true,
		LoadingTitle = "Aether Interface Suite",
	...
	LoadingFrame.Version.Text = LoadingFrame.Frame.Frame.Title.Text == "Aether Interface Suite" and Release or "Aether UI"
	...
	if WindowSettings.KeySystem then
		...
		Draggable(AetherUI.MobileSupport, AetherUI.MobileSupport)
		...
		local direc = WindowSettings.KeySettings.SaveInRoot and "Aether/Configurations/" .. WindowSettings.ConfigSettings.RootFolder .. "/" .. WindowSettings.ConfigSettings.ConfigFolder .. "/Key System/" or "Aether/Configurations/" ..  WindowSettings.ConfigSettings.ConfigFolder .. "/Key System/"

		if isfile and isfile(direc .. WindowSettings.KeySettings.FileName .. ".Aether") then
			for i, Key in ipairs(WindowSettings.KeySettings.Key) do
				if string.find(readfile(direc .. WindowSettings.KeySettings.FileName .. ".Aether"), Key) then
					Passthrough = true
					break
				end
			end
			...
			KeySystem.Action.Submit.Interact.MouseButton1Click:Connect(function()
				...
				if KeyFound then 
					...
					if WindowSettings.KeySettings.SaveKey then
						if writefile then
							writefile(direc .. WindowSettings.KeySettings.FileName .. ".Aether", FoundKey)
						end
						Aether:Notification({Title = "Key System", Content = "The key for this script has been saved successfully.", Icon = "lock_open"})
					end
				else
	...
	function Window:CreateHomeTab(HomeTabSettings)
		...
		for i,v in pairs(HomeTabSettings.SupportedExecutors) do
			if isStudio then HomeTabPage.detailsholder.dashboard.Client.Subtitle.Text = "Aether Interface Suite - Debugging Mode" break end
	...
	function Window:CreateTab(TabSettings)
		...
		function Tab:CreateSection(name : string)
			...
			function Section:CreateButton(ButtonSettings)
				...
				Button.Interact["MouseButton1Click"]:Connect(function()
					...
					if not Success then
						...
						print("Aether Interface Suite | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Button.Title.Text = ButtonSettings.Name
						TweenService:Create(Button, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
			...
			function Section:CreateSlider(SliderSettings, Flag)
						...
						if SLDragging then
							...
							if SliderSettings.CurrentValue ~= NewValue then
								...
								if not Success then
									...
									print("Aether Interface Suite | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
									wait(0.5)
									Slider.Title.Text = SliderSettings.Name
									TweenService:Create(Slider, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
				...
				if Flag then
					Aether.Options[Flag] = SliderV
				end

				AetherUI.ThemeRemote:GetPropertyChangedSignal("Value"):Connect(function()
					Slider.Main.color.Color = Aether.ThemeGradient
					Slider.Main.UIStroke.color.Color = Aether.ThemeGradient
				end)
			...
			function Section:CreateToggle(ToggleSettings, Flag)    
				...
				Toggle.Interact.MouseButton1Click:Connect(function()
					...
					if not Success then
						...
						print("Aether Interface Suite | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Toggle.Title.Text = ToggleSettings.Name
						TweenService:Create(Toggle, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
				...
				AetherUI.ThemeRemote:GetPropertyChangedSignal("Value"):Connect(function()
					Toggle.toggle.color.Color = Aether.ThemeGradient
					Toggle.toggle.UIStroke.color.Color = Aether.ThemeGradient
				end)

				if Flag then
					Aether.Options[Flag] = ToggleV
				end
			...
			function Section:CreateBind(BindSettings, Flag)
					...
					if CheckingForKey then
						if input.KeyCode ~= Enum.KeyCode.Unknown and input.KeyCode ~= Window.Bind then
							...
							if not Success then
								...
								print("Aether Interface Suite | "..BindSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Bind.Title.Text = BindSettings.Name
				...
				if Flag then
					Aether.Options[Flag] = BindV
				end

				-- Aether.Flags[BindSettings.Flag] = BindSettings
					...
					if InputSettings.Enter then
						if bleh then
							...
							if not Success then
								...
								print("Aether Interface Suite | "..InputSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Input.Title.Text = InputSettings.Name
			...
			function Section:CreateDropdown(DropdownSettings, Flag)
				...
				local function SafeCallback(param, c2)
					...
					if not Success then
						...
						print("Aether Interface Suite | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Dropdown.Title.Text = DropdownSettings.Name
			...
			function Section:CreateColorPicker(ColorPickerSettings, Flag) -- by Rayfield/Throit
				...
				local function SafeCallback(param, c2)
					...
					if not Success then
						...
						print("Aether Interface Suite | "..ColorPickerSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						ColorPicker.Title.Text = ColorPickerSettings.Name
				...
				if Flag then
					Aether.Options[Flag] = ColorPickerV
				end
		...
		function Tab:BuildConfigSection()
			...
			Tab:CreateButton({
				...
				Callback = function()
					...
					local success, returned = Aether:SaveConfig(inputPath)
					if not success then
						Aether:Notification({
							Title = "Interface",
							Icon = "error",
					...
					configSelection:Set({ Options = Aether:RefreshConfigList() })
				end
			})
			...
			configSelection = Tab:CreateDropdown({
				...
				Options = Aether:RefreshConfigList(),
				CurrentOption = {},
				MultipleOptions = false,
				SpecialType = nil,
				Callback = function(Value)
			...
			Tab:CreateButton({
				...
				Callback = function()
					local success, returned = Aether:LoadConfig(selectedConfig)
					if not success then
						Aether:Notification({
							Title = "Interface",
							Icon = "error",
			...
			Tab:CreateButton({
				...
				Callback = function()
					local success, returned = Aether:SaveConfig(selectedConfig)
					if not success then
						Aether:Notification({
							Title = "Interface",
							Icon = "error",
			...
			Tab:CreateButton({
				...
				Callback = function()
					...
					writefile(Aether.Folder .. "/settings/autoload.txt", name)
			...
			Tab:CreateButton({
				...
				Callback = function()
					local name = selectedConfig
					delfile(Aether.Folder .. "/settings/autoload.txt")
			...
			if isfile(Aether.Folder .. "/settings/autoload.txt") then
				local name = readfile(Aether.Folder .. "/settings/autoload.txt")
				loadlabel:Set( { Text = "Current autoload config: " .. name })
			end     
		end
		...
		local ClassParser = {
			["Toggle"] = {
				...
				Load = function(Flag, data)
					if Aether.Options[Flag] then
						Aether.Options[Flag]:Set({ CurrentValue = data.state })
					end
				end
			},
			["Slider"] = {
				...
				Load = function(Flag, data)
					if Aether.Options[Flag] and data.value then
						Aether.Options[Flag]:Set({ CurrentValue = data.value })
					end
				end
			},
			["Input"] = {
				...
				Load = function(Flag, data)
					if Aether.Options[Flag] and data.text and type(data.text) == "string" then
						Aether.Options[Flag]:Set({ CurrentValue = data.text })
					end
				end
			},
			["Dropdown"] = {
				...
				Load = function(Flag, data)
					if Aether.Options[Flag] and data.value then
						Aether.Options[Flag]:Set({ CurrentOption = data.value })
					end
				end
			},
			-- buggy as hell stil
			["Colorpicker"] = {
				...
				Load = function(Flag, data)
					...
					if Aether.Options[Flag] and data.color then
						Aether.Options[Flag]:Set({Color = HexToColor3(data.color)})
					end
				end
			}
		}
		...
		function Tab:BuildThemeSection()
			...
			}, "AetherInterfaceSuitePrebuiltCPC1") -- A flag is the identifier for the configuration file, make sure every element has a different flag if you're using configuration saving to ensure no overlaps

			local c2cp = Tab:CreateColorPicker({
				Name = "Color 2",
				Color = Color3.fromRGB(123, 201, 201),
			}, "AetherInterfaceSuitePrebuiltCPC2")

			local c3cp = Tab:CreateColorPicker({
				Name = "Color 3",
				Color = Color3.fromRGB(224, 138, 184),
			}, "AetherInterfaceSuitePrebuiltCPC3") 
			...
			c1cp:Set({
				Callback = function(Value)
					if c2cp and c3cp then
						Aether.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Value or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(0.50, c2cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1.00, c3cp.Color or Color3.fromRGB(255,255,255))}
						AetherUI.ThemeRemote.Value = not AetherUI.ThemeRemote.Value
					end
				end
			})
			...
			c2cp:Set({
				Callback = function(Value)
					if c1cp and c3cp then
						Aether.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, c1cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(0.50, Value or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1.00, c3cp.Color or Color3.fromRGB(255,255,255))}
						AetherUI.ThemeRemote.Value = not AetherUI.ThemeRemote.Value
					end
				end
			})
			...
			c3cp:Set({
				Callback = function(Valuex)
					if c2cp and c1cp then
						Aether.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, c1cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(0.50, c2cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1.00, Valuex or Color3.fromRGB(255,255,255))}
						AetherUI.ThemeRemote.Value = not AetherUI.ThemeRemote.Value
					end
				end
			})
		...
		local function SetFolder()
			...
			if WindowSettings.ConfigSettings.RootFolder ~= nil and WindowSettings.ConfigSettings.RootFolder ~= "" then
				Aether.Folder = WindowSettings.ConfigSettings.RootFolder .. "/" .. WindowSettings.ConfigSettings.ConfigFolder
			else
				Aether.Folder = WindowSettings.ConfigSettings.ConfigFolder
			end
		...
		function Aether:SaveConfig(Path)
			if isStudio then return "Config system unavailable." end
			...
			local fullPath = Aether.Folder .. "/settings/" .. Path .. ".Aether"

			local data = {
				objects = {}
			}

			for flag, option in next, Aether.Options do
		...
		function Aether:LoadConfig(Path)
			if isStudio then return "Config system unavailable." end
			...
			local file = Aether.Folder .. "/settings/" .. Path .. ".Aether"
		...
		function Aether:LoadAutoloadConfig()
			if isfile(Aether.Folder .. "/settings/autoload.txt") then
				...
				local success, err = Aether:LoadConfig(name)
				if not success then
					return Aether:Notification({
						Title = "Interface",
						Icon = "sparkle",
						ImageSource = "Material",
		...
		function Aether:RefreshConfigList()
			if isStudio then return "Config system unavailable." end

			local list = listfiles(Aether.Folder .. "/settings")

			local out = {}
			for i = 1, #list do
				local file = list[i]
				if file:sub(-5) == ".Aether" then
					local pos = file:find(".Aether", 1, true)
					local start = pos
	...
	Main.Controls.Close.ImageLabel.MouseButton1Click:Connect(function()
		...
		if UserInputService.KeyboardEnabled == false then
			AetherUI.MobileSupport.Visible = true
		end
	end)
	...
	UserInputService.InputBegan:Connect(function(input, gpe)
		...
		if input.KeyCode == Window.Bind then
			...
			AetherUI.MobileSupport.Visible = false
	...
	AetherUI.MobileSupport.Interact.MouseButton1Click:Connect(function()
		Unhide(Main, Window.CurrentTab)
		dragBar.Visible = true
		Window.State = true
		AetherUI.MobileSupport.Visible = false
...
if isStudio then
	local Window = Aether:CreateWindow({
		Name = "Nebula Client - Aether Hub | Blade Ball",
		Subtitle = "by Nebula Softworks",
		LogoID = "123795201100198",
		LoadingEnabled = true,
		LoadingTitle = "Nebula Client (Aether Hub)",
	...
	Aether:Notification({ 
		...
		Icon = "sparkle",
		ImageSource = "Material",
		Content = "Welcome to the Aether Interface Suite. This Is an Amazing Quality Freemium UI Library For Roblox Exploiting Made By Nebula Softworks. Aether was Created in hopes of improving the standard of UI Library designs by being the golden standard for it. Aether Has Amazing Features like a key system, notification and perfection in aesthetics and design. So, What Are You Waiting For? Start Using Aether Today at " .. website
	})
	...
	Tabs.Main2:CreateDropdown({
		...
		Description = "Aether's Dropdowns Has a built in Player Dropdown!",
...
--[[if isStudio then
    window = Aether:CreateWindow({LoadingEnabled = false})
    t1 = window:CreateTab()
    ...
    t1:CreateParagraph({Text = "Welcome to the Aether Interface Suite. This Is an Amazing Quality Freemium UI Library For Roblox Exploiting Made By Nebula Softworks. Aether was Created in hopes of improving the standard of UI Library designs by being the golden standard for it. Aether Has Amazing Features like a key system, notification and perfection in aesthetics and design. So, What Are You Waiting For? Start Using Aether Today at " .. website})
